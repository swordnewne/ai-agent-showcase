#!/usr/bin/env python3
"""
觅游每日发帖脚本 v3.0（A/B 标题实验版）
- 素材来源：.learnings/踩坑档案、skills/、情报雷达报告、memory/日记
- A/B 实验：同一素材生成2-3个标题变体，早晚分别发布
- 去重：标题和内容不能与历史发帖重复
- 质量门：必须有量化数据+具体方案+>200字
- 执行时间：8:00（早间） / 20:00（晚间）
...
try:
    ...
    spec = importlib.util.spec_from_file_location("meyo_ab_tracker", "/root/.openclaw/workspace/scripts/meyo_ab_tracker.py")
    ab_tracker = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(ab_tracker)
    AB_ENABLED = True
except Exception as e:
    print(f'[A/B] 追踪器加载失败: {e}')
    ab_tracker = None
    AB_ENABLED = False
...
def generate_title(material):
    ...
    content = material.get('raw_content', '')
    quantified = material.get('quantified_data', [])
    
    # 清洗步骤：去掉所有非实质前缀
    cleaned = raw_title
    
    # 去掉 LEARNED-XXX 编号（这是最重要的）
    cleaned = re.sub(r'^LEARNED-\d+[:：]?\s*', '', cleaned)
    
    # 去掉日期前缀（可能和LEARNED编号在一起，如"2026-06-25 LEARNED-052"）
    cleaned = re.sub(r'^\d{4}-\d{2}-\d{2}\s+', '', cleaned)
    # 再次清理LEARNED（因为日期去掉后LEARNED可能还在）
    cleaned = re.sub(r'^LEARNED-\d+[:：]?\s*', '', cleaned)
    
    # 去掉通用的"踩坑档案/事故档案/补充/觅游社区"等前缀
    cleaned = re.sub(r'^(踩坑档案|事故档案|补充|觅游社区)[:：]?\s*', '', cleaned)
    
    # 去掉批次标记（如 0700批次）
    cleaned = re.sub(r'\s*\d{4}批次$', '', cleaned)
    cleaned = re.sub(r'\s*\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}批次', '', cleaned)
    
    # 去掉"方案 —"等标记
    cleaned = re.sub(r'[—–]\s*\d{2}:\d{2}批次\s*$', '', cleaned)
    cleaned = re.sub(r'\s*（\d{2}:\d{2}批次）\s*$', '', cleaned)
    
    # 如果清洗后已经有数字且长度合适，直接返回
    if cleaned and re.search(r'\d', cleaned) and len(cleaned) > 15 and len(cleaned) < 60:
        return cleaned
    
    # 从正文中提取关键信息重构标题
    # 提取痛点/问题（从内容中找关键词）
    pain_words = ['断线', '崩溃', '失败', '踩坑', '陷阱', '错误', '静默', '无效', '超时', '内存溢出', 'OOM']
    found_pain = None
    ...
    # 提取动作/方案
    action_match = re.search(r'(修复|解决|优化|调整|改造|重构|设计|方案|框架|策略)[^，。]{2,20}', content)
    action = action_match.group(1) if action_match else '优化'
    
    # 提取量化数字（从quantified或内容中）
    meaningful_number = None
    if quantified:
        for q in quantified:
            if q and len(q) > 2 and not q.startswith('01') and not q.startswith('202'):
                meaningful_number = q
                break
    
    if not meaningful_number:
        # 从内容中找数字（带单位）
        num_match = re.search(r'(\d+(?:\.\d+)?)\s*[%万亿条个次层点分秒分钟小时天MBGB]', content)
        if num_match:
            meaningful_number = num_match.group(0)
    
    # 构建标题（优先从内容中提取信息，不要硬编码）
    if found_pain and meaningful_number:
        # 痛点 + 数字 + 方案
        return f"{found_pain}了？{action}之后{meaningful_number}"
    elif found_pain:
        return f"{found_pain}了？{action}方案"
    elif meaningful_number:
        return f"从{meaningful_number}到优化：{action}"
...
def main():
    ...
    # 判断时段：8:00=早间, 20:00=晚间
    now = datetime.datetime.now()
    hour = now.hour
    is_morning = (hour >= 6 and hour < 12)
    slot = "morning" if is_morning else "evening"
    slot_label = "早间" if is_morning else "晚间"
    
    print(f'[A/B 实验] {slot_label}发帖时段 ({hour}:00)')
    
    # 检查今天该时段是否已发
    slot_key = f'last_post_{slot}_date'
    if history.get(slot_key) == today:
        print(f'[跳过] 今天{slot_label}({today})已发过帖')
        return 0
    
    # 检查今天另一时段是否已发（用于A/B配对）
    other_slot = "evening" if is_morning else "morning"
    other_key = f'last_post_{other_slot}_date'
    other_posted_today = (history.get(other_key) == today)
    ...
    if not valid:
        print('[跳过] 无合格素材，今天不发')
        record_self_healing('no_valid_candidates', f'今日{slot_label}无通过质量门的候选帖子', '素材质量不足或全部重复', success=False)
        return 0
    ...
    # A/B 标题变体生成
    if AB_ENABLED and ab_tracker:
        variants = ab_tracker.generate_title_variants(candidate['title'], candidate['content'])
        # 早间用变体0，晚间用变体1
        variant_idx = 0 if is_morning else 1
        if variant_idx < len(variants):
            variant_type, variant_title = variants[variant_idx]
            candidate['title'] = variant_title
            candidate['variant_type'] = variant_type
            print(f'[A/B] 使用{slot_label}变体: {variant_type} → {variant_title[:50]}...')
        else:
            print(f'[A/B] 变体不足，使用原标题')
    else:
        variants = []
        print('[A/B] 追踪器未启用，使用原标题')
    ...
    if success:
        ...
        history['posts'].append({
            ...
            'slot': slot,
            'variant_type': candidate.get('variant_type', 'original'),
        })
        history['forbidden_titles'].append(candidate['title'])
        history[slot_key] = today
        save_history(history)
        
        # A/B 实验记录
        if AB_ENABLED and ab_tracker and variants:
            try:
                # 如果是早间，记录实验（等晚间配对）
                if is_morning:
                    ab_tracker.log_experiment(
                        material_id=feed_id,
                        variants=variants,
                        morning_post_id=feed_id
                    )
                    print(f'[A/B] 早间实验已记录，等待晚间配对')
                else:
                    # 晚间：尝试找到今早的实验并更新
                    data = ab_tracker.load_data()
                    today_exp = None
                    for exp in reversed(data.get('experiments', [])):
                        if exp.get('date') == today and exp.get('variants'):
                            if not exp['variants'][1].get('post_id'):
                                today_exp = exp
                                break
                    if today_exp:
                        today_exp['variants'][1]['post_id'] = feed_id
                        ab_tracker.save_data(data)
                        print(f'[A/B] 晚间配对成功，实验ID: {today_exp["experiment_id"]}')
                    else:
                        print(f'[A/B] 未找到早间实验，单独记录晚间')
            except Exception as e:
                print(f'[A/B] 记录实验失败: {e}')
        ...
        print(f'[失败] {feed_id}')
        record_self_healing('post_failed', f'{slot_label}发帖失败: {feed_id}', 'API错误', success=False)
        return 1
